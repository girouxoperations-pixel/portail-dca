...
function EntryRow({ e, profileMap, csmByEntryId, recurringIds, occurrenceId, isAdmin, onEdit, onDelete, onAnnulerRecu, onRefund, pending }: {
  ...
  const isPartialRefund   = !isRefunded && (e.collected ?? 0) > 0 && (e.collected ?? 0) < (e.montant_courant ?? 0)
  const montantRembourse  = isPartialRefund ? Math.round(((e.montant_courant ?? 0) - (e.collected ?? 0)) * 100) / 100 : 0
  ...
  if (isPartialRefund) {
    return (
      <tr className="bg-orange-50/40 border-l-4 border-orange-300">
        ...
        <td className="px-4 py-3">
          <span className="inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full bg-orange-100 text-orange-700 whitespace-nowrap">
            Remb. partiel
          </span>
        </td>
        ...
        <td className="px-4 py-3 text-right tabular-nums text-blue-700 whitespace-nowrap">
          <div>{dollar(e.collected)}</div>
          <div className="text-[10px] text-orange-500 font-medium">−{dollar(montantRembourse)} remb.</div>
        </td>
